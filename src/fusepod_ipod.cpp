...
string FUSEPod::discover_ipod() {
    ...
    while (fin) {
        ...
        if (dev == "fusepod") {
            ...
            while (stats) {//TODO Check if this causes a crash
                ...
                if (!fusepod_starts_with(line.c_str(),
                                         kRealMountpointPrefix.c_str()))
                    continue;

                // Gets the path of the mount point
                string mountpoint = line.substr(kRealMountpointPrefix.size());
                already_mounted.insert(mountpoint);
                break;
            }
            stats.close();
        }
    ...
    for (vector<string>::iterator it = possible.begin(); it != possible.end();
         ++it) {
    ...
    if (possible.size () > 0) {
        cout << "WARNING: iPod at " << possible[0]
             << " has already been mounted by FUSEPod." << endl;
        return possible[0];
    }
...
bool fusepod_create_directory(const string &path) {
    const char *s = path.c_str();
    return access(s, F_OK) != 0 && mkdir(s, 0777) != 0;
}
...
bool FUSEPod::create_itunes_dirs(const string &dir) {
    ...
    if (fusepod_create_directory(control))
        return false;

    string itunes = control + "/iTunes";
    if (fusepod_create_directory(itunes))
        return false;

    string music = control + "/Music";
    if (fusepod_create_directory(music))
        return false;
    ...
    for (int i = 0; i < 20; i++) {
        ...
        if (fusepod_create_directory(music_dir))
            return false;
    }
...
bool FUSEPod::remove_song(const string &path) {
    ...
    /* TODO There has to be a better way than this to remove the track from
     * the playlists in fusepod. */
...
void FUSEPod::remove_playlist(const string &name) {
    for (GList *i = this->ipod->playlists; i; i = i->next) {
        ...
        const char * pname = fusepod_get_string(
            fusepod_check_string(playlist->name).c_str());
...
Node *FUSEPod::get_node(const char *path) {
    ...
    for (size_t i = 0; i < paths.size () && cur; i++) {
        node.text = paths[i];
        cur = cur->find(node);
    }
...
string FUSEPod::get_transfer_path(const char *path) {
    return mount_point + '/' + dir_transfer_ipod +
        &(path[dir_transfer.size() + 1]);
}
...
string FUSEPod::get_statistics () {
    ...
    stats << "FUSEPod Version: " << PACKAGE_VERSION << endl
          << "ITunesDB Version: " << this->ipod->version << endl
          << kRealMountpointPrefix << itdb_get_mountpoint(this->ipod) << endl
          << "iPod Name: " << (mpl && mpl->name ? string(mpl->name) : "Unknown")
          << endl
          << "Track Count: " << this->num_tracks << endl;
...
string FUSEPod::get_track_val(Track *track, char symbol) {
    ...
    switch (symbol) {
        ...
        case 't': // Title
            if (track->compilation) {
                val = string(track->artist) + " - " + string(track->title);
            } else if (track->title) {
                val = track->title;
            }
            break;
...
string FUSEPod::expand_string(Track *track, const string &format) {
    ...
    if (tmp == tokens[0])
        ...
        ret += fusepod_check_string(get_track_val(track, tokens[0][0])) +
            (&tokens[0][1]);

    for (size_t a = 1; a < tokens.size(); a++) {
        ret += fusepod_check_string(get_track_val(track, tokens[a][0])) +
            (&tokens[a][1]);
    }
...
void FUSEPod::add_track(Track *track, const string &path_desc) {
    ...
    for (size_t i = 0; i < paths.size(); i++) {
        ...
        NodeValue nv(
            fusepod_get_string(fusepod_check_string(path.c_str()).c_str()),
            MODE_DIR);
        if (i == paths.size() - 1) {
            nv.mode = MODE_FILE;
            nv.track = track;
...
void FUSEPod::remove_track(Track *track, const string &path_desc) {
    ...
    if (node != 0 && node != root && !S_ISDIR(node->value.mode)) {
        // Deletes all directories that will become empty.
...
void FUSEPod::add_playlists() {
    ...
    for (GList *i = this->ipod->playlists; i; i = i->next) {
        ...
        for (GList *a = playlist->members; a; a = a->next) {
            ...
            string filename = pos + " - " + fusepod_check_string(
                expand_string(track, playlist_track_format));

            node->addChild(NodeValue(fusepod_get_string(filename.c_str()),
                                     MODE_FILE, track, track->size));
        }
    }
}
...
bool FUSEPod::move_file(const string &path, Track *track) {
    ...
    sprintf(tmp, ":iPod_Control:Music:F%02d:fusepod%06d.%s",
            dirnum, filenum, ext.c_str());
